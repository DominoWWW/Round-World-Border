...
$forceload add $(nether_x) $(nether_z)
$summon marker $(nether_x) ~ $(nether_z) {Tags:["rwb.particle"]}
execute at @n[tag=rwb.particle] run tp @n[tag=rwb.particle] ~ ~ ~ facing entity @s
$execute at @n[tag=rwb.particle] run tp @n[tag=rwb.particle] ^ ^ ^$(nether_radius)
...
$execute if data storage round_world_border:data {chunk:0b} run forceload remove $(nether_x) $(nether_z)